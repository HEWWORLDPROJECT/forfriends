...
if type(hookfunction) == "function" and type(ClientFire) == "table" and volleyRoute and type(ClientFire[volleyRoute]) == "function" then
    ...
    nativeFireVolley = hookfunction(ClientFire[volleyRoute], hookClosure(function(tool, muzzleIndex, bulletIndex, origin, directions, options)
        ...
        if State.active and State.noRecoil and type(directions) == "table" and #directions > 0 then
            local base = directions[1]
            if typeof(base) == "Vector3" and base.Magnitude > 0.001 then
                local flat = base.Unit
            ...
            if type(options) == "table" then
                ...
                for key, val in pairs(options) do
                    ...
                    if string.find(lk, "recoil", 1, true)
                        or string.find(lk, "spread", 1, true)
                        or string.find(lk, "bloom", 1, true)
                        or string.find(lk, "kick", 1, true)
                        or string.find(lk, "dispersion", 1, true) then
                        if type(val) == "number" then
                            options[key] = 0
                        elseif typeof(val) == "Vector3" then
                            options[key] = Vector3.zero
                        elseif typeof(val) == "Vector2" then
                            options[key] = Vector2.zero
            ...
            Camera = Workspace.CurrentCamera or Camera
            if Camera then
                State._nrLook = Camera.CFrame.LookVector
                State._nrUntil = os.clock() + 0.18
            end
        end
        if State.active and State.silentAim and type(directions) == "table" and #directions > 0 then
            ...
            if data then
                ...
                if vector.Magnitude > 0.001 then
                    ...
                    local aimDir = vector.Unit * math.max(vector.Magnitude, 1)
                    redirected[1] = aimDir
                    if State.noRecoil then
                        for i = 1, #redirected do
                            redirected[i] = aimDir
                        end
                    end
                    directions = redirected
                    if State.noRecoil and type(options) == "table" then
                        local o = table_clone(options)
                        for _, key in ipairs({"Recoil", "recoil", "Spread", "spread", "Bloom", "bloom", "CameraKick", "cameraKick"}) do
                            if o[key] ~= nil then
                                if type(o[key]) == "number" then
                                    o[key] = 0
                                elseif typeof(o[key]) == "Vector3" then
                                    o[key] = Vector3.zero
                                elseif typeof(o[key]) == "Vector2" then
                                    o[key] = Vector2.zero
                                end
                            end
                        end
...
if type(hookfunction) == "function" and type(WeaponSource) == "table"
    ...
    local ok, err = pcall(function()
        nativeToFireParams = hookfunction(WeaponSource.toFireParams, hookClosure(function(params)
            if State.active and State.noRecoil and type(params) == "table" then
                params = table_clone(params)
                for _, key in ipairs({"Spread", "spread", "Recoil", "recoil", "Bloom", "bloom"}) do
                    if type(params[key]) == "number" then params[key] = 0 end
                end
            end
            local result = nativeToFireParams(params)
            if State.active and State.noRecoil and type(result) == "table" then
                for _, key in ipairs({"Spread", "spread", "Recoil", "recoil", "Bloom", "bloom"}) do
                    if type(result[key]) == "number" then result[key] = 0 end
                end
            end
...
local function drawLockTriangle(center, height, color)
    for index = 1, 3 do
        if lockHalos[index] then lockHalos[index].Visible = false end
        if lockLines[index] then lockLines[index].Visible = false end
    end
end
            ...
            if not root or not head then
                ...
                if rootScreen.Z <= 0 then
                    ...
                    if State.visibility then
                        hasLos = rawLineOfSight(head, localOrigin())
                    end
                    local baseColor = (selected and hasLos) and (State.espColorSelected or Colors.accent) or (State.espColor or Colors.enemy)
                    color = baseColor
                    visual.box.Color = color
                    ...
                    -- lock mark disabled (false positive diamonds)
...
local ACCENT_DIM = Color3.fromRGB(88, 60, 160)
local ACCENT_SOFT = Color3.fromRGB(196, 168, 255)
...
local STROKE_SOFT = Color3.fromRGB(36, 36, 48)
local TEXT = Color3.fromRGB(240, 240, 245)
local MUTED = Color3.fromRGB(140, 142, 160)
local TRACK = Color3.fromRGB(32, 32, 42)
local SUCCESS = Color3.fromRGB(52, 211, 153)
local DANGER = Color3.fromRGB(248, 113, 113)
...
local function label(parent, text, size, color, bold)
    ...
    t.Size = UDim2.new(1, 0, 0, (size or 13) + 6)
    t.Parent = parent
    return t
end
...
root.Position = UDim2.new(0.5, -380, 0.5, -250)
root.BackgroundColor3 = BG
root.BorderSizePixel = 0
root.Parent = screen
corner(root, 12)
stroke(root, ACCENT_DIM, 1.5)
...
statusLabel.Position = UDim2.new(1, -200, 0, 0)
statusLabel.Size = UDim2.fromOffset(184, 44)
...
content.Size = UDim2.new(1, -28, 1, -110)
content.Position = UDim2.fromOffset(14, 96)
...
footerHint.Size = UDim2.new(1, -24, 1, 0)
footerHint.Position = UDim2.fromOffset(14, 0)
footerHint.Font = Enum.Font.Gotham
footerHint.TextSize = 11
footerHint.TextXAlignment = Enum.TextXAlignment.Left
footerHint.TextColor3 = MUTED
footerHint.Text = "Tip: Settings → Menu Bind to change open key"
footerHint.Parent = footer

task.spawn(function()
    while screen.Parent and State.active do
        statusLabel.Text = tostring(State.lastResult or "ready")
        task.wait(0.35)
    end
end)
...
local function makeTab(name)
    ...
    btn.Parent = tabBar
    corner(btn, 8)
    stroke(btn, STROKE_SOFT, 1)

    btn.MouseEnter:Connect(function()
        if activeTab ~= name then
            tween(btn, {BackgroundColor3 = CARD_HOVER}, 0.12)
        end
    end)
    btn.MouseLeave:Connect(function()
        if activeTab ~= name then
            tween(btn, {BackgroundColor3 = CARD}, 0.12)
        end
    end)
    ...
    left.Parent = page
    corner(left, 10)
    stroke(left, STROKE_SOFT, 1)
    pad(left, 12, 12, 12, 12)
    ...
    right.Parent = page
    corner(right, 10)
    stroke(right, STROKE_SOFT, 1)
    pad(right, 12, 12, 12, 12)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        btn.TextColor3 = ACCENT_SOFT
        btn.BackgroundColor3 = Color3.fromRGB(36, 28, 52)
        page.Visible = true
        activeTab = name
        footerHint.Text = "Tab: " .. name .. "  ·  status updates in the top-right"
    end)
...
local function section(parent, titleText)
    ...
    titleLbl.TextColor3 = ACCENT_SOFT
...
State._uiRefresh = State._uiRefresh or {}
local function addToggle(parent, name, get, set)
    ...
    local function paint()
        ...
        if on then
            ...
            knob.Position = UDim2.fromOffset(21, 3)
        else
            track.BackgroundColor3 = TRACK
            knob.Position = UDim2.fromOffset(3, 3)
        end
    ...
    paint()
    table.insert(State._uiRefresh, paint)
...
local function refreshAllUI()
    if type(State._uiRefresh) ~= "table" then return end
    for _, fn in ipairs(State._uiRefresh) do
        pcall(fn)
    end
end
...
local function addSlider(parent, name, minV, maxV, step, get, set, suffix)
    ...
    local function refresh()
        ...
        top.Text = name
        valueLbl.Text = string.format("%s%s", tostring(v), suffix)
    end
    refresh()
    table.insert(State._uiRefresh, refresh)
...
local function addDropdown(parent, name, options, get, set)
    ...
    btn.MouseEnter:Connect(function()
        tween(btn, {BackgroundColor3 = CARD_HOVER}, 0.1)
    end)
    btn.MouseLeave:Connect(function()
        tween(btn, {BackgroundColor3 = TRACK}, 0.1)
    end)
    return row
end
...
local function addButton(parent, name, callback)
    ...
    stroke(btn, STROKE_SOFT, 1)
    btn.MouseEnter:Connect(function()
        tween(btn, {BackgroundColor3 = ACCENT_DIM, TextColor3 = ACCENT_SOFT}, 0.12)
    end)
    btn.MouseLeave:Connect(function()
        tween(btn, {BackgroundColor3 = TRACK, TextColor3 = TEXT}, 0.12)
    end)
    btn.MouseButton1Click:Connect(function()
        pcall(callback)
    end)
...
local function addColor(parent, name, get, set)
    ...
    sw.Size = UDim2.fromOffset(32, 20)
    sw.Position = UDim2.new(1, -32, 0.5, -10)
...
local CONFIG_KEYS = {
    "profile", "menuKeyName", "silentAim", "noRecoil", "esp", "espBox", "espCorner", "espSkeleton", "espChams",
...
local function importConfig(raw)
    ...
    raw = tostring(raw):gsub("^%s+", ""):gsub("%s+$", ""):gsub('^"+', ""):gsub('"+$', "")
    if not string.find(raw, "CW1", 1, true) then
        State.lastResult = "invalid config"
        return false
    end
    local startPos = string.find(raw, "CW1", 1, true)
    if startPos then raw = string.sub(raw, startPos) end
    local map = {}
    ...
    local boolKeys = {
        silentAim=true, noRecoil=true, esp=true, espBox=true, espCorner=true, espSkeleton=true, espChams=true,
        showNames=true, showDistance=true, showHealthBar=true, showWeapon=true, showFov=true,
        tracers=true, triggerbot=true, visibility=true, prediction=true, smartTargeting=true,
        angleAudit=true, penetrationAudit=true, autoDefense=true,
        vehicleSpeedEnabled=true, vehicleFly=true, vehicleSpin=true, vehicleNoclip=true, autoRoadkill=true,
    }
    local numKeys = {
        fovRadius=true, aimDistance=true, espDistance=true, tracerDistance=true, triggerRadius=true,
        triggerDelay=true, vehicleTargetSpeed=true, legitDwell=true, legitMaxAngle=true, defenseThreshold=true,
    }
    for k, v in pairs(map) do
        if keySet[k] == "color" then
            State[k] = strToColor(v)
        elseif boolKeys[k] then
            State[k] = (v == "1" or v == "true" or v == "True")
        elseif numKeys[k] then
            local n = tonumber(v)
    ...
    if type(State.menuKeyName) == "string" and #State.menuKeyName > 0 then
        local ok, code = pcall(function()
            return Enum.KeyCode[State.menuKeyName]
        end)
        if ok and code then
            State.menuKeyCode = code
        end
    end
...
local function readClipboard()
    local env = (getgenv and getgenv()) or _G
    for _, name in ipairs({"getclipboard", "get_clipboard", "readclipboard", "GetClipboard"}) do
    ...
    local okSyn, syn = pcall(function() return rawget(env, "syn") or rawget(_G, "syn") end)
    if okSyn and type(syn) == "table" and type(syn.get_clipboard or syn.clipboard_get) == "function" then
        local fn = syn.get_clipboard or syn.clipboard_get
        local ok, val = pcall(fn)
        if ok and type(val) == "string" and #val > 0 then return val end
    end
    return nil
...
local function importFromString(raw)
    return importConfig(raw)
end
...
do
    ...
    addToggle(s, "Enabled", function() return State.silentAim end, function(v) State.silentAim = v end)
    addToggle(s, "No Recoil", function() return State.noRecoil end, function(v) State.noRecoil = v end)
...
do
    ...
    do
        ...
        wrap.BackgroundColor3 = TRACK
        wrap.BorderSizePixel = 0
        wrap.ClipsDescendants = true
        wrap.Parent = s
        corner(wrap, 5)
        stroke(wrap, STROKE, 1)

        local row = Instance.new("TextBox")
        ...
        row.TextSize = 11
        row.TextColor3 = TEXT
        row.PlaceholderText = "Paste CW1 config here"
        row.PlaceholderColor3 = MUTED
        row.ClearTextOnFocus = false
        row.Text = ""
        ...
        row.Parent = wrap
        row:GetPropertyChangedSignal("Text"):Connect(function()
            if #row.Text > 0 then
                State.importBuffer = row.Text
            end
        end)
        importBox = row
    end
    addButton(s, "Import Config", function()
        ...
        if (not raw or #raw < 4) then
            raw = State.importBuffer
        end
        if (not raw or #raw < 4) and importBox and #importBox.Text > 4 then
            raw = importBox.Text
        end
        if raw and #raw > 3 then
            if importConfig(raw) then
                State.importBuffer = ""
                if importBox then
                    importBox.Text = ""
                end
            ...
            State.lastResult = "clipboard empty — paste into box"
        end
    end)
...
do
    ...
    menuBindBtn.Size = UDim2.new(1, 0, 0, 28)
    menuBindBtn.BackgroundColor3 = TRACK
    menuBindBtn.BorderSizePixel = 0
    menuBindBtn.Font = Enum.Font.Gotham
    menuBindBtn.TextSize = 12
    menuBindBtn.TextColor3 = TEXT
    menuBindBtn.AutoButtonColor = true
    menuBindBtn.Text = "Menu Bind: " .. tostring(State.menuKeyName or "Delete")
    menuBindBtn.Parent = s
    corner(menuBindBtn, 5)
    stroke(menuBindBtn, STROKE, 1)
    local function refreshMenuBindLabel()
        if State.waitingMenuKey then
            menuBindBtn.Text = "Press any key..."
            menuBindBtn.TextColor3 = ACCENT
        else
            menuBindBtn.Text = "Menu Bind: " .. tostring(State.menuKeyName or "Delete")
            menuBindBtn.TextColor3 = TEXT
        end
    ...
    State._uiRefresh = State._uiRefresh or {}
    table.insert(State._uiRefresh, refreshMenuBindLabel)
    menuBindBtn.MouseButton1Click:Connect(function()
        State.waitingMenuKey = true
        refreshMenuBindLabel()
    end)
...
tabs["Legit"].TextColor3 = ACCENT_SOFT
tabs["Legit"].BackgroundColor3 = Color3.fromRGB(36, 28, 52)
pages["Legit"].page.Visible = true
activeTab = "Legit"
footerHint.Text = "Tab: Legit  ·  open/close with your Menu Bind key"
...
table.insert(State.connections, UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if State.waitingMenuKey then
        if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode ~= Enum.KeyCode.Unknown then
            ...
            State.menuKeyName = tostring(input.KeyCode):gsub("Enum.KeyCode.", "")
            State.waitingMenuKey = false
            State.lastResult = "menu bind: " .. State.menuKeyName
            if refreshAllUI then pcall(refreshAllUI) end
            return
        end
        return
    ...
    local bind = State.menuKeyCode or Enum.KeyCode.Delete
    if input.KeyCode == bind then
        menuVisible = not menuVisible
        root.Visible = menuVisible
        State.panelVisible = menuVisible
    end
end))
...
table.insert(State.connections, RunService.RenderStepped:Connect(function(deltaTime)
    ...
    if State.noRecoil and State._nrLook and os.clock() < (State._nrUntil or 0) then
        Camera = Workspace.CurrentCamera or Camera
        if Camera then
            local pos = Camera.CFrame.Position
            local cur = Camera.CFrame.LookVector
            local target = State._nrLook
            local mixed = cur:Lerp(target, 0.65)
            if mixed.Magnitude > 0.001 then
                Camera.CFrame = CFrame.new(pos, pos + mixed.Unit)
            end
        end
    end
